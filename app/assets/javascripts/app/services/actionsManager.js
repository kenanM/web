...
import { Action, SFModelManager, protocolManager } from 'snjs';
    ...
    let handleResponseDecryption = async (response, keys, merge) => {
      ...
      if(!item.errorDecrypting) {
        ...
        for(let passwordCandidate of this.previousPasswords) {
          ...
          var keyResults = await protocolManager.computeRootKey({password: passwordCandidate, keyParams: response.auth_params});
          if(!keyResults) {
            continue;
          }
